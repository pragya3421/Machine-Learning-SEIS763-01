...
new_df=pd.concat([df,Gender_dummy,Smoker_dummy,Location_dummy,Self_dum
my],axis=1)
new_df.drop(labels=['Gender','Smoker','Location',"'Female'",0,'SelfAss
essedHealthStatus',
"'St. Mary's Medical
Center'","'Poor'"],axis=1,inplace=True)
new_df.rename(columns={1:'Smoker',"'Male'":'Male',"'county General
Hospital'":'countyGenralHospital', "'VA
Hospital'":'VAHospital',"'Excellent'":'Excellent',
"'Fair'":'Fair',"'Good'":'Good'},inplace=True)
X= new_df.drop(['Systolic'],axis=1)
Y=new_df['Systolic']
X_train,X_test,y_train,y_test=train_test_split(X,Y,test_size=0.033,ran
dom_state=42)
...
ymin,ymax=plt.ylim()
plt.vlines(-
np.log10(model.alpha_),ymin,ymax,linestyle='dotted',label='Selected
alpha')
plt.legend()
